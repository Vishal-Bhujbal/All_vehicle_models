...
baseline_results = Generate_GGV_Map(modelName, sus_par_base, par_base);
...
for p = 1:num_params
    ...
    for level = [1, 3] % Test only Low (1) and High (3) [Base (2) is already done]
        ...
        % 3. Update BOTH structs to ensure the parameter actually changes
        if isfield(sus_par_test, p_name)
            sus_par_test.(p_name) = test_val;
        end
        ...
        % 4. Push the FULL updated structs to the base workspace (for workspace clarity/debugging)
        assignin('base', 'sus_par', sus_par_test);
        assignin('base', 'Par', Par_test);
        
        % 5. Generate GGV Map explicitly passing local structs to sandbox
        ggv_results = Generate_GGV_Map(modelName, sus_par_test, Par_test);
        
        % 6. Process Envelope and Calculate Lap Time (Optional, kept for log)
        ggv_env = Process_GGV_Envelope(ggv_results);
        lap_res = Solve_3Pass(path_s, path_k, ggv_env);
        
        % 7. Store & Calculate % Change for Max Roll
