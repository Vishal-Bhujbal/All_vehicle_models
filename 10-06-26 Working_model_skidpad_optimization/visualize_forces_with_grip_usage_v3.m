function interactive_vehicle_viewer_pro
    ...
    N = length(time);
    try 
        steer_L = squeeze(out.steer_L.Data); 
        if length(steer_L) ~= N; steer_L = zeros(N,1); end
    catch
        steer_L = zeros(N,1); 
    end
    
    try 
        steer_R = squeeze(out.steer_R.Data); 
        if length(steer_R) ~= N; steer_R = zeros(N,1); end
    catch
        steer_R = zeros(N,1); 
    end
    ...
    % =========================================================================
    % Bottom control panel divider (y = 0.12)
    annotation(fig, 'line', [0.02 0.98], [0.12 0.12], 'Color', line_sep, 'LineWidth', 1.5);
    % Vertical divider (Starts exactly ON the bottom line at y = 0.12)
    annotation(fig, 'line', [0.60 0.60], [0.12 0.95], 'Color', line_sep, 'LineWidth', 1.5);
    % Horizontal quadrant divider (Starts exactly ON the vertical line at x = 0.60)
    annotation(fig, 'line', [0.60 0.98], [0.55 0.55], 'Color', line_sep, 'LineWidth', 1.5);

    % =========================================================================
    % AXES 1: TRAJECTORY (TOP RIGHT - WITH PADDING)
    % =========================================================================
    ...
    axis(ax1, 'equal'); % Apply equal ratio AFTER limits are set
    ...
    % =========================================================================
    % Shifted bottom edge up to 0.18 to prevent label overlap with the 0.12 line
    ...
    % =========================================================================
    % Shifted bottom edge up to 0.18
    ...
    function update_frame(idx)
        ...
        % NEGATIVE SIGN APPLIED: Flips steering logic for proper visual rotation
        ...
        for k = 1:4
            ...
            force_lines(k).XData = [0, Fy(k)*force_scale];
            force_lines(k).YData = [0, Fx(k)*force_scale];
