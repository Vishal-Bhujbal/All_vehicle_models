function interactive_vehicle_viewer_pro
    ...
    % Trajectory HUD (Shifted past the grid)
    traj_coord_hud = annotation(fig, 'textbox', [0.84 0.88 0.14 0.05], ...
        'String', '', 'Color', white_col, 'EdgeColor', 'none', ...
        'FontSize', 10, 'FontWeight', 'bold', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top');
    
    % Static limits restored for full track view
    ...
    % Yaw Rate HUD
    yaw_hud = annotation(fig, 'textbox', [0.45 0.85 0.15 0.1], ...
        'String', '', 'Color', white_col, 'EdgeColor', 'none', ...
    ...
    % =========================================================================
    % Base Position shifted up to 0.18 to prevent xlabel intersection with white line
    % Bounding box size perfectly matches Trajectory for symmetric UI
    ax3_pos = [0.65 0.18 0.28 0.32]; 
    ...
    % Explicit padding to mimic Trajectory aesthetic (15% padding vertically, balanced horizontally)
    ...
    function update_frame(idx)
        ...
        for k = 1:4
            ...
            % Plot force vectors using Body Frame forces
