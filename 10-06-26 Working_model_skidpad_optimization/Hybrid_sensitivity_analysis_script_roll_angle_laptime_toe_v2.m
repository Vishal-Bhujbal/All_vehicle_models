% =========================================================================
% HYBRID SENSITIVITY SCRIPT (V12 - LAP TIME + MAX ROLL ANGLE)
% Purpose: OFAT Springs/ARBs + Multi-Level Camber/Toe + Damper Sweep
...
% Convert static angles from degrees to radians for the baseline
...
% Updated to target Toe_f_static (and Toe_r_static if applicable)
if isfield(sus_par_base, 'Toe_f_static')
    sus_par_base.Toe_f_static = deg2rad(sus_par_base.Toe_f_static);
end
if isfield(sus_par_base, 'Toe_r_static')
    sus_par_base.Toe_r_static = deg2rad(sus_par_base.Toe_r_static);
end
if isfield(par_base, 'Toe_f_static')
    par_base.Toe_f_static = deg2rad(par_base.Toe_f_static);
end
if isfield(par_base, 'Toe_r_static')
    par_base.Toe_r_static = deg2rad(par_base.Toe_r_static);
end
...
%% --- 2. PHASE 1: OFAT SWEEP (SPRINGS, ARBS ±30%) ---
fprintf('=== STARTING PHASE 1: SPRINGS & ARBS (±30%%) ===\n');
param_names = {'Ks_f', 'Ks_r', 'K_arb_f', 'K_arb_r'};
...
for p = 1:num_params
    ...
    for level = 1:2
        ...
        if isfield(sus_par_test, p_name), sus_par_test.(p_name) = test_val; end
        if isfield(Par_test, p_name),     Par_test.(p_name)     = test_val; end
...
%% --- 3. PHASE 2: MULTI-LEVEL SWEEP (CAMBER & TOE) ---
fprintf('\n=== STARTING PHASE 2: ALIGNMENT ANGLES (CAMBER & TOE) ===\n');
camber_vals = -4:1:2;  % -4 to +2 deg
toe_vals    = -3:1:3;  % -3 to +3 deg

pct_change_time_camber = zeros(length(camber_vals), 1);
pct_change_roll_camber = zeros(length(camber_vals), 1);
pct_change_time_toe    = zeros(length(toe_vals), 1);
pct_change_roll_toe    = zeros(length(toe_vals), 1);

% --- Camber Sweep ---
for i = 1:length(camber_vals)
    test_val = camber_vals(i);
    model_test_val = deg2rad(test_val);
    
    sus_par_test = sus_par_base;
    Par_test = par_base;
    if isfield(sus_par_test, 'Camber_static'), sus_par_test.Camber_static = model_test_val; end
    if isfield(Par_test, 'Camber_static'),     Par_test.Camber_static     = model_test_val; end
    
    fprintf('Testing Camber = %2d deg... ', test_val);
    simIn = Simulink.SimulationInput(modelName);
    simIn = simIn.setModelParameter('SimulationMode', 'normal');
    simIn = simIn.setVariable('sus_par', sus_par_test);
    simIn = simIn.setVariable('Par', Par_test);
    try
        ...
        laptime = getScalarTime(out, 't2') - getScalarTime(out, 't1');
        maxroll = getMaxRoll(out);
        pct_change_time_camber(i) = ((laptime - base_laptime) / base_laptime) * 100;
        pct_change_roll_camber(i) = ((maxroll - base_roll) / base_roll) * 100;
        fprintf('Time Δ: %+.4f %% | Roll Δ: %+.4f %%\n', pct_change_time_camber(i), pct_change_roll_camber(i));
    catch
        fprintf('FAILED\n');
        pct_change_time_camber(i) = NaN;
        pct_change_roll_camber(i) = NaN;
    end
end

% --- Toe Sweep ---
for i = 1:length(toe_vals)
    test_val = toe_vals(i);
    model_test_val = deg2rad(test_val);
    ...
    % Targets Toe_f_static explicitly (and Toe_r_static if defined in your parameter set)
    if isfield(sus_par_test, 'Toe_f_static'), sus_par_test.Toe_f_static = model_test_val; end
    if isfield(sus_par_test, 'Toe_r_static'), sus_par_test.Toe_r_static = model_test_val; end
    if isfield(Par_test, 'Toe_f_static'),     Par_test.Toe_f_static     = model_test_val; end
    if isfield(Par_test, 'Toe_r_static'),     Par_test.Toe_r_static     = model_test_val; end
    
    fprintf('Testing Toe = %2d deg...    ', test_val);
    simIn = Simulink.SimulationInput(modelName);
    simIn = simIn.setModelParameter('SimulationMode', 'normal');
    simIn = simIn.setVariable('sus_par', sus_par_test);
    simIn = simIn.setVariable('Par', Par_test);
    try
        ...
        laptime = getScalarTime(out, 't2') - getScalarTime(out, 't1');
        maxroll = getMaxRoll(out);
        pct_change_time_toe(i) = ((laptime - base_laptime) / base_laptime) * 100;
        pct_change_roll_toe(i) = ((maxroll - base_roll) / base_roll) * 100;
        fprintf('Time Δ: %+.4f %% | Roll Δ: %+.4f %%\n', pct_change_time_toe(i), pct_change_roll_toe(i));
    catch
        fprintf('FAILED\n');
        pct_change_time_toe(i) = NaN;
        pct_change_roll_toe(i) = NaN;
    end
end

%% --- 4. PHASE 3: EXPLICIT 15-SETUP DAMPER SWEEP ---
fprintf('\n=== STARTING PHASE 3: 15 DISCRETE DAMPER SETUPS ===\n');
...
% --- Camber - Lap Time ---
figure('Name', 'Camber Sweep - Lap Time', 'Color', 'w', 'Position', [100 600 400 350]);
plot(camber_vals, pct_change_time_camber, '-ob', 'LineWidth', 1.5); 
ylabel('Lap Time \Delta (%)'); xlabel('Camber Angle (deg)');
title('Camber Angle vs Lap Time'); grid on;

% --- Camber - Max Roll Angle ---
figure('Name', 'Camber Sweep - Max Roll', 'Color', 'w', 'Position', [520 600 400 350]);
plot(camber_vals, pct_change_roll_camber, '-sr', 'LineWidth', 1.5); 
ylabel('Max Roll \Delta (%)'); xlabel('Camber Angle (deg)');
title('Camber Angle vs Max Roll'); grid on;

% --- Toe - Lap Time ---
figure('Name', 'Toe Sweep - Lap Time', 'Color', 'w', 'Position', [940 600 400 350]);
plot(toe_vals, pct_change_time_toe, '-ob', 'LineWidth', 1.5); 
ylabel('Lap Time \Delta (%)'); xlabel('Toe Angle (deg)');
title('Toe Angle vs Lap Time'); grid on;

% --- Toe - Max Roll Angle ---
figure('Name', 'Toe Sweep - Max Roll', 'Color', 'w', 'Position', [1360 600 400 350]);
plot(toe_vals, pct_change_roll_toe, '-sr', 'LineWidth', 1.5); 
ylabel('Max Roll \Delta (%)'); xlabel('Toe Angle (deg)');
title('Toe Angle vs Max Roll'); grid on;

% --- Heatmaps for Springs/ARBs ---
x_labels_cont = {'Low (-30%)', 'High (+30%)'};
y_labels_cont = {'Front Spring', 'Rear Spring', 'Front ARB', 'Rear ARB'};
y_labels_damp = {configs.name}';
x_label_damp = {'Performance Delta (%)'};

figure('Name', 'Springs & ARBs - Lap Time', 'Color', 'w', 'Position', [100 150 400 350]);
h1 = heatmap(x_labels_cont, y_labels_cont, pct_change_time_cont);
h1.Title = sprintf('Lap Time: Springs/ARBs Δ%%\nBase Lap: %.4f s', base_laptime);
h1.CellLabelFormat = '%+.4f %%'; h1.MissingDataColor = [0.15 0.15 0.15]; h1.Colormap = jet; 
balanceColorLimits(h1, pct_change_time_cont);

figure('Name', 'Springs & ARBs - Max Roll', 'Color', 'w', 'Position', [520 150 400 350]);
h2 = heatmap(x_labels_cont, y_labels_cont, pct_change_roll_cont);
h2.Title = sprintf('Max Roll: Springs/ARBs Δ%%\nBase Roll: %.6f', base_roll);
h2.CellLabelFormat = '%+.4f %%'; h2.MissingDataColor = [0.15 0.15 0.15]; h2.Colormap = turbo; 
balanceColorLimits(h2, pct_change_roll_cont);

% --- Heatmaps for Dampers ---
figure('Name', 'Dampers - Lap Time', 'Color', 'w', 'Position', [940 150 400 450]);
...
figure('Name', 'Dampers - Max Roll Angle', 'Color', 'w', 'Position', [1360 150 400 450]);
