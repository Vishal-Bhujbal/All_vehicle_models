function interactive_vehicle_viewer_pro
    ...
    % =========================================================================
    fig = figure( ...
        'Name', 'FSAE Pro Telemetry Viewer (Real-Time)', ...
        'Color', bg_color, ...
        'Renderer', 'opengl', ...
    ...
    % =========================================================================
    % 1:1 REAL-TIME PLAY BUTTON
    % =========================================================================
    % Moved to the right side and widened since the speed menu is gone
    ...
    % =========================================================================
    % KEYBOARD SUPPORT (Scrubbing Timeline)
    % =========================================================================
    set(fig, 'WindowKeyPressFcn', @key_press);
    function key_press(~, event)
        % If currently playing, ignore keystrokes to prevent conflicts
    ...
    % =========================================================================
    % REAL-TIME SYNCHRONIZATION PLAYBACK FUNCTION
    % =========================================================================
    function play_callback(src, ~)
        if get(src, 'Value')
            src.String = 'PAUSE';
            
            % Capture the exact system time when PLAY was pressed
            sys_start_time = tic;
            % Capture the simulation time we are starting from
            sim_start_time = time(current_idx);
            
            while get(src, 'Value')
                % How much physical time has elapsed since we hit play?
                elapsed_sys_time = toc(sys_start_time);
                
                % The simulation time we SHOULD be at
                target_sim_time = sim_start_time + elapsed_sys_time;
                
                % Fast forward our index to catch up to the target time
                ...
                if temp_idx >= N
                    ...
                    % If they hit replay at the end, reset to the beginning
                    src.Callback = @replay_wrapper;
                    break;
                end
                
                % Only render if the index actually moved forward
                if temp_idx > current_idx
                    update_frame(temp_idx);
                else
                    % Yield processing power back to the computer if we are waiting 
                    % for the next timestamp (prevents freezing the UI)
                    pause(0.005);
                end
            end
        else
    ...
    % Helper to loop back to the start if the user presses replay at the end
    function replay_wrapper(src, event)
        if current_idx == N
            current_idx = 1;
            update_frame(1);
        end
        src.Callback = @play_callback; 
        play_callback(src, event);
    end
